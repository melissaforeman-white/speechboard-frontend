...
function CRUDBoard(props) {
    // let navigate = useNavigate()
    const location = useLocation()
    // location state
    const action = location.state.action
    let id, title, cards, boardSize = null
    // contexts
    ...
    if (location.state.action === 'create') {
        ...
        title = location.state.boardData.title
        cards = location.state.boardData.cards
    }   
    ...
    const createPreviewBoard = () => {
        return (
            <Board boardData= {{'id': id, 'title':boardTitle, 'cards':cardsData, 'selectedCard': selectedCard, 'setSelectedCard': setSelectedCard}}></Board>
        )
    }
